...
(define-test test-random-forest
    (let (syobu bc-train bc-test cars forest query)
      ...
      (setf syobu (read-data-from-file (asdf:system-relative-pathname 'clml.extras "sample/syobu.csv") :type :csv :csv-type-spec '(string integer integer integer integer)))
      
      (setf bc-train (read-data-from-file (asdf:system-relative-pathname 'clml.extras "sample/bc.train.csv") :type :csv 
					  :csv-type-spec (append (loop for i below 9 collect 'double-float) '(string))))
      
      (setf bc-test (read-data-from-file (asdf:system-relative-pathname 'clml.extras "sample/bc.test.csv") :type :csv 
					 :csv-type-spec (append (loop for i below 9 collect 'double-float) '(string))))
      
      (setf cars (read-data-from-file (asdf:system-relative-pathname 'clml.extras "sample/cars.csv") :type :csv :csv-type-spec '(double-float double-float)))
