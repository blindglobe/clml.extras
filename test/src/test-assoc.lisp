...
(define-test test-sample-assoc
    (let (dataset result result1
      ...
      (assert-true
       (setf result (association-analyze (asdf:system-relative-pathname 'clml.extras "sample/pos.sexp") (asdf:system-relative-pathname 'clml.extras "sample/result.sexp")
                                         '("商品名") "ID番号" 3 
      ...
      (loop with rules = (assoc-result-rules result)
          for rule in 
            (with-open-file (in (asdf:system-relative-pathname 'clml.extras "sample/result.sexp") :external-format #+allegro :932 #+(and (not ccl) (not allegro)) :sjis  #+ccl :Windows-31j) (read in))
          for i from 0
          do (if (= i 0) 
                 (assert-true 
      ...
      (assert-true 
       (setf dataset (read-data-from-file (asdf:system-relative-pathname 'clml.extras "sample/pos.sexp") :external-format #+allegro :932 #+(and (not ccl) (not allegro)) :sjis  #+ccl :Windows-31j)))
